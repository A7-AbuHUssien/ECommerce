...
public class ProductService : IProductService
{
    ...
    public async Task<IEnumerable<Product>> GetAsync(bool tracked = true,
    ...
    public IQueryable<Product> GetQueryable(bool tracked = true)
    {
    ...
    public async Task<Product?> GetProductByIdAsync(int id, CancellationToken cancellationToken = default)
    {
        return await _productRepository.GetOneAsync(
            p => p.Id == id,
            includes: [p => p.Brand, p => p.Category, p => p.SubImages, p => p.Colors],
            tracked: true,
            cancellationToken: cancellationToken
        );
    }

    public async Task<Product> CreateAsync(Product product, IFormFile? mainImg, List<IFormFile>? subImages,
        string[]? colors, CancellationToken cancellationToken = default)
    {
        if (mainImg != null)
    ...
    public async Task EditAsync(Product product, IFormFile? mainImg, List<IFormFile>? subImages, string[]? colors,
        CancellationToken cancellationToken = default)
    {
        var oldProduct = await _productRepository.GetOneAsync(x => x.Id == product.Id, tracked: false, cancellationToken: cancellationToken);

        if (mainImg != null)
            product.MainImg = await HandleImgAsync(mainImg, ["wwwroot", "Uploads", "Product"]);
        else
    ...
    {
        var product = await _productRepository.GetOneAsync(expression: p => p.Id == productId, tracked: false);

        if (product == null)
        {
            return Enumerable.Empty<Product>().AsQueryable();
        }
        var related = await _productRepository.GetAsync(
            expression: e => e.Name.Contains(product.Name) && e.Id != productId,
    ...
    public async Task<bool> DeleteAsync(int id, CancellationToken cancellationToken = default)
    {
        ...
        var subImages =
            await _subImageRepository.GetAsync(p => p.ProductId == id, cancellationToken: cancellationToken);
    ...
    public IQueryable<Product> ApplyFilters(IQueryable<Product> products, AdminProductFilterVM? filter)
    {
        ...
        if (filter.QuantityUnder > 0)
            products = products.Where(p => p.Quantity <= filter.QuantityUnder);

        return products;
    }
    public async Task<(IEnumerable<Product> Products, int TotalItems)> PaginateAsync
        (
    ...
    private async Task HandleSubImagesAsync(Product product, List<IFormFile>? subImages)
    {
        ...
        {
            var filename = await HandleImgAsync(subfile,
                new List<string> { "wwwroot", "Uploads", "Product", "ProductSubImg" });
